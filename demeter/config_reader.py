...
class ReadConfig:
    ...
    def __init__(self, ini_file):
        ...
        self.out_dir = self.get_outdir(s['out_dir'], False)
        ...
        self.log_dir = self.create_dir(os.path.join(self.out_dir, o['log_dir']), self.log, True)
        ...
        self.lc_per_step_csv = self.create_dir(os.path.join(self.out_dir, o['lc_per_step_csv']), self.log, True)
    ...
    @staticmethod
    def create_dir(d, log, make=False):
        """
        Create directory.
    ...
    def get_outdir(self, out, verbose=True):
        """
        ...
        if verbose:
            ...
            v = self.config['STRUCTURE']['out_dir']

        print v

        # create run specific directory matching the format used to create the log file
        return self.create_dir(os.path.join(pth, v), self.log, True)
