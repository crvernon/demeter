...
class RandomConfig:
    ...
    def selection(self):
        ...
        # use priority file as-is and keep it static
        prir = []
        with open(self.priority_file, 'rU') as get:
            get.next()
            for i in get:
        ...
        nav = np.arange(0.0, 1.1, 0.1)

        # soil workability
        swrk = np.arange(0.0, 1.1, 0.1)

        # kernel weight
        kwgt = np.arange(0.0, 1.1, 0.1)

        # create a unique combination of each type
        l = []
        ...
        for i in range(self.perms):
            ...
            trt, iv, st, kd = l[sct]
            l.pop(sct)
            self.mix.append([pri, trt, round(iv, 2), round(st, 2), kd, i])
